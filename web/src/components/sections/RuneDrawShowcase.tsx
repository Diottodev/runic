...
export function RuneDrawShowcase() {
  const { t } = useLocale()
  const { ref: headerRef, inView: headerIn } = useInView()
  const { ref: demoRef, inView: demoIn } = useInView(0.15)

  return (
    <section id="runedraw" className="py-20 md:py-28 border-t border-border/50">
      <div className="container max-w-6xl">
        ...
        >
          <div className="rounded-2xl border border-border/70 bg-card overflow-hidden card-lift">
            ...
            {/* Diagram */}
            <iframe
              src="/runedraw-demo.html?embed=1"
              title={t.runedraw.demo_caption}
            ...
            <div className="flex items-center gap-3 px-4 py-3 border-t border-border/60 bg-muted/30">
              <p className="text-xs text-muted-foreground leading-relaxed">
                {t.runedraw.demo_caption}
              </p>
              <a
                href="/runedraw-demo.html"
                target="_blank"
