...
function usage() {
  return `Usage:
  runedraw render <type> <input.json> [output.html] [--quality standard|showcase] [--repo-root path]
  runedraw compare architecture <base.json> <head.json> [output.html] [--receipt path] [--json] [--quality standard|showcase] [--repo-root path]
  runedraw deliver <type> <input.json> [output.html] [--json] [--open] [--quality standard|showcase] [--repo-root path]
  runedraw preview <type> <input.json> [output.html] [--no-open] [--quality standard|showcase] [--repo-root path]
  runedraw validate <type> <input.json> [--json] [--layout-json] [--quality standard|showcase] [--repo-root path]
  runedraw inspect <type> <input.json>
  runedraw check <output.html>
  runedraw guide [scenario or question] [--json] [--lang en|zh]
  runedraw examples
  runedraw doctor
  runedraw demo [output-directory]
...
function rendererEnv(quality, repoRoot, diagnosticJson = false) {
  return {
    ...(quality ? { RUNEDRAW_QUALITY_PROFILE: quality } : {}),
    ...(repoRoot ? { RUNEDRAW_REPO_ROOT: repoRoot } : {}),
    ...(diagnosticJson ? { RUNEDRAW_DIAGNOSTIC_FORMAT: 'json' } : {}),
  };
}
...
async function commandCompare(args) {
  ...
  try {
    ...
    reportCompareFailure({ json: options.json, stage: 'prepare', error: 'Architecture compare runtime is unavailable.', code: 'delta/runtime-missing', details: { reason: error.message, supportedFixes: ['install the complete RuneDraw skill package'] } });
    return;
  }
  const {
  ...
  try {
    stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.runedraw-compare-'));
  } catch (error) {
...
function sourceEvidenceFromArtifact(artifact) {
  ...
  const match = html.match(/<script id="runedraw-source-evidence-data" type="application\/json">([\s\S]*?)<\/script>/);
...
async function commandDeliver(args) {
  ...
  let stagingDirectory;
  try {
    stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.runedraw-delivery-'));
  } catch (error) {
...
async function commandDoctor() {
  ...
  console.error(`\nRuneDraw is not ready: ${problems.join('; ')}.`);
  process.exitCode = 1;
}
...
function commandDemo(args) {
  ...
  console.log('  runedraw render architecture <input.json> <output.html>');
}
...
function commandValidate(args) {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-validate-'));
