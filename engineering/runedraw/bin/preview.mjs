...
const cliPath = path.join(here, 'runedraw.mjs');
...
<head>
  ...
  <title>RuneDraw Live Preview</title>
  <style>
...
<body data-state="checking" data-has-artifact="false">
  <header>
    <span class="brand">RuneDraw Preview</span>
  ...
  <main>
    ...
    <iframe id="artifact" title="Verified RuneDraw diagram"></iframe>
  </main>
  <script>
    (function () {
      'use strict';
...
export async function startPreview(options) {
  ...
  const stagingDirectory = fs.mkdtempSync(path.join(outputDirectory, '.runedraw-preview-'));
    ...
    state.failure = {
      ...
      message: redactDiagnostic(
        ...
        [
          ...
          [path.resolve(here, '..'), '<runedraw-skill>'],
