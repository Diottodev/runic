...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-delta-'));
...
test('compare CLI writes a deterministic three-state artifact and complete sidecar receipt', () => {
  ...
  assert.match(firstHtml, /window\.RuneDraw\.deltaExport = \{ canonicalSvg: canonicalDeltaSvg, shareCard/);
  ...
  assert.match(firstHtml, /document\.querySelectorAll\('#runedraw-compare-receipt'\)\.length !== 1/);
...
test('artifact validation fails closed on missing, duplicate, or self-blessed review identity', () => {
  ...
  const receiptNode = html.match(/<script id="runedraw-compare-receipt"[\s\S]*?<\/script>/)?.[0];
