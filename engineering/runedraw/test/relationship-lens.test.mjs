...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-relationship-lens-'));
...
test('relationship lens groups incoming, outgoing, and self-loop paths and follows neighbors', () => {
  ...
  assert.match(html, /RuneDraw\.view\.reveal\(\[id\], \{ includeNeighbors: true, reason: 'relationship' \}\)/);
