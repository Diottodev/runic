...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-guided-views-'));
...
for (const [mode, config] of Object.entries(CASES)) {
  test(`${mode}: guided views preserve base SVG geometry`, () => {
    ...
    assert.match(guided.html, /RuneDraw\.guidedViews = \(function \(\)/);
    ...
    assert.match(guided.html, /--runedraw-scroll-x/);
