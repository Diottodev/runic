...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-semantic-lens-'));
...
test('all typed renderers inherit one viewer-only Semantic Lens', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /RuneDraw\.semanticLens = \(function \(\)/, mode);
...
test('Semantic Lens is shareable and yields cleanly to stronger reader intent', () => {
  ...
  assert.match(html, /RuneDraw\.semanticLens\.clear\(\{ updateUrl: false/);
  assert.match(html, /RuneDraw\.focus\.clear\(\{ updateUrl: false, preserveView: true \}\)/);
  assert.match(html, /RuneDraw\.routeProbe\.clear\(\{ updateUrl: false, restoreFocus: false \}\)/);
  assert.match(html, /RuneDraw\.guidedViews\.showAll\(\{ clearFocus: false, updateUrl: false \}\)/);
  assert.match(html, /if \(action === 'lens'\) return RuneDraw\.semanticLens\.open\(\)/);
  assert.match(html, /e\.key === 'l' \|\| e\.key === 'L'/);
  assert.match(html, /e\.key === 'Escape' && RuneDraw\.semanticLens\.isOpen\(\)/);
  assert.match(html, /e\.key === 'Escape' && RuneDraw\.semanticLens\.active\(\)/);
});
