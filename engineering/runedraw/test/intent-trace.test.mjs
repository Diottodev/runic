...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-intent-trace-'));
...
test('all typed renderers inherit one geometry-neutral Intent Trace', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /RuneDraw\.intentTrace = \(function \(\)/, mode);
...
test('Intent Trace separates hover, keyboard, touch, and committed focus', () => {
  ...
  assert.match(html, /RuneDraw\.focus\.active\(\)/);
  assert.match(html, /RuneDraw\.intentTrace\.clear\(\{ announce: false \}\)/);
  assert.match(html, /e\.key === 'Escape' && RuneDraw\.intentTrace\.active\(\)/);
});
...
test('Intent Trace normalizes motion, respects reduced motion, and exports cleanly', () => {
  ...
  assert.match(html, /@keyframes runedraw-intent-trace-flow/);
  assert.match(html, /animation: runedraw-intent-trace-flow 1\.15s linear 1 both/);
