...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-chapter-handoff-'));
...
test('handoff holds one truthful anchor then settles through one finite camera transaction', () => {
  ...
  assert.match(html, /RuneDraw\.motionGovernor\.claim\('handoff'/);
  assert.match(html, /RuneDraw\.motionGovernor\.release\(handoff\.ownerToken\)/);
...
test('latest intent, manual takeover, Still, reduced motion, and hidden pages cleanly settle', () => {
  ...
  assert.match(html, /RuneDraw\.guidedViews\.cancelHandoff\(reason \|\| 'manual'\)/);
