...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-semantic-radar-'));
...
test('all typed renderers inherit one viewer-only Semantic Radar', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /RuneDraw\.radar = \(function \(\)/, mode);
...
test('Semantic Radar derives semantic node bounds and focuses stable IDs', () => {
  ...
  assert.match(html, /RuneDraw\.view\.reveal\(\[id\], \{ includeNeighbors: true, reason: 'radar' \}\)/);
...
test('Semantic Radar tracks desktop camera and mobile contained scroll', () => {
  ...
  assert.match(html, /chip\.style\.top = Math\.round\(top\) \+ 'px';[\s\S]+RuneDraw\.radar\.sync\(\)/);
  ...
  assert.match(html, /--runedraw-radar-top/);
...
test('Semantic Radar keeps redundant accessible navigation and clean exports', () => {
  ...
  assert.match(html, /e\.key === 'Escape' && RuneDraw\.radar\.isOpen\(\)/);
