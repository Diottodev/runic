// Golden-file harness for the runedraw renderers. No test framework needed:
...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-test-'));
...
for (const [mode, input, golden] of GOLDEN) {
  ...
  try {
    ...
    check(`${mode}: packaged ${golden}`, normalizeNewlines(fresh) === normalizeNewlines(packaged),
      `fresh render differs from runedraw/examples/${golden}; re-render the packaged examples and rebuild runedraw.zip`);
  } catch (err) {
...
check('template generator meta matches package.json version',
  template.includes(`<meta name="generator" content="runedraw ${pkg.version}">`),
