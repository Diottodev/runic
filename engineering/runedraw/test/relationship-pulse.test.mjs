...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-relationship-pulse-'));
...
test('semantic tokens use five distinct inline SVG cues on one finite timing owner', () => {
  ...
  assert.match(html, /animation: runedraw-relationship-token-life 1\.2s linear 1 both/);
  assert.match(html, /semantic-flow-token-halo/);
  assert.match(html, /RuneDraw\.flowTokens = \{/);
...
test('pulse is finite, event-owned, preset-aware, touch-safe, and motion-safe', () => {
  ...
  assert.match(html, /animation: runedraw-relationship-pulse 1\.2s linear 1 both/);
  assert.match(html, /@keyframes runedraw-relationship-token-life/);
  ...
  assert.match(html, /RuneDraw\.motionGovernor && RuneDraw\.motionGovernor\.isPaused\(\)/);
