...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-route-share-card-'));
...
test('Route Share Card menu lifecycle excludes hidden state from keyboard navigation', () => {
  ...
  assert.match(html, /function clear\(options\)[\s\S]*?RuneDraw\.exportMenu\.syncRouteShare\(\)/);
  assert.match(html, /function showResult\(result, options\)[\s\S]*?RuneDraw\.exportMenu\.syncRouteShare\(\)/);
...
test('Route Share Card reuses one 1200x630 variant seam and publishes a truthful receipt', () => {
  ...
  assert.match(html, /var snapshot = RuneDraw\.routeProbe && RuneDraw\.routeProbe\.exportSnapshot\(\)/);
  ...
  assert.match(html, /clearExportReceipt\(\);[\s\S]*?var snapshot = RuneDraw\.routeProbe/);
...
test('skill and READMEs describe the optional Export variant and show one real card without changing the hero', () => {
  ...
  for (const readme of ['README.md', 'README_EN.md', 'README_ZH.md']) {
    ...
    assert.match(text, /docs\/assets\/runedraw-route-share-card\.png/, readme);
  }

  const png = fs.readFileSync(path.join(repoRoot, 'docs/assets/runedraw-route-share-card.png'));
  assert.equal(png.subarray(0, 8).toString('hex'), '89504e470d0a1a0a');
  assert.equal(png.readUInt32BE(16), 1200);
  assert.equal(png.readUInt32BE(20), 630);

  const buildZip = fs.readFileSync(path.join(repoRoot, 'scripts/build-zip.sh'), 'utf8');
  assert.match(buildZip, /--exclude '\.validator-check-\*'/, 'concurrent validator scratch must never leak into runedraw.zip');
});
