...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-finder-'));
...
test('all typed renderers ship the same geometry-neutral node finder', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /RuneDraw\.finder = \(function \(\)/, mode);
    assert.match(html, /svg\.querySelectorAll\('\[data-node-id\]'\)/, mode);
    assert.doesNotMatch(svg(html), /node-finder|RuneDraw\.finder|Find a node/, mode);
  }
});
...
test('finder searches semantic ids and labels, then delegates to focus and reveal', () => {
  ...
  assert.match(html, /RuneDraw\.guidedViews\.showAll\(\{ clearFocus: false, updateUrl: false \}\)/);
  assert.match(html, /RuneDraw\.view\.reset\(\{ automatic: true \}\)/);
  assert.match(html, /RuneDraw\.focus\.set\(id, \{ toggle: false \}\)/);
  assert.match(html, /RuneDraw\.view\.reveal\(\[id\], \{ includeNeighbors: true, reason: 'finder' \}\)/);
...
test('finder becomes a contextual Route Probe endpoint picker without changing semantic focus', () => {
  ...
  assert.match(html, /RuneDraw\.routeProbe\.finderContext\(\)/);
  ...
  assert.match(html, /RuneDraw\.routeProbe\.choose\(id\)/);
