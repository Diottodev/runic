...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-animation-'));
...
test('signal-flow preset reaches the page, SVG, and motion export surface', () => {
  ...
  assert.match(html, /RuneDraw\.motion = \{ canRecord: canRecordMotion, recordWebm: recordWebm \}/);
  assert.match(html, /recorder\.requestData\(\)/);
  assert.match(html, /aria-label="Diagram view controls"/);
  assert.match(html, /RuneDraw\.focus = \(function \(\)/);
  assert.match(html, /RuneDraw\.view = \(function \(\)/);
...
for (const [mode, example] of Object.entries(CASES)) {
  test(`${mode}: trace animation annotates svg, edges, and nodes`, () => {
    ...
    assert.match(svg, /aria-labelledby="runedraw-diagram-title runedraw-diagram-description"/);
    assert.match(svg, /<title id="runedraw-diagram-title">[^<]+<\/title>/);
    assert.match(svg, /<desc id="runedraw-diagram-description">[^<]+<\/desc>/);
