...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-cli-'));
const cli = path.join(skillRoot, 'bin/runedraw.mjs');
...
fs.writeFileSync(process.env.RUNEDRAW_TEST_OPEN_LOG, JSON.stringify({
...
test('cli: help lists commands and diagram types', () => {
  ...
  assert.match(result.stdout, /runedraw render <type>/);
  assert.match(result.stdout, /runedraw compare architecture <base\.json> <head\.json>/);
  assert.match(result.stdout, /runedraw deliver <type>/);
  assert.match(result.stdout, /runedraw preview <type>/);
  assert.match(result.stdout, /--open/);
  assert.match(result.stdout, /runedraw guide \[scenario or question\]/);
  assert.match(result.stdout, /runedraw doctor/);
  assert.match(result.stdout, /runedraw demo \[output-directory\]/);
...
test('cli: doctor reports a complete installation is ready', () => {
  ...
  assert.match(result.stdout, /RuneDraw is ready\./);
});
...
test('cli: doctor identifies an incomplete installation', () => {
  ...
  fs.copyFileSync(cli, path.join(incompleteBin, 'runedraw.mjs'));

  const result = spawnSync(process.execPath, [path.join(incompleteBin, 'runedraw.mjs'), 'doctor'], {
    cwd: incompleteRoot,
    encoding: 'utf8',
  });
  ...
  assert.match(result.stderr, /RuneDraw is not ready: \d+ required files? missing\./);
});
...
test('cli: doctor rejects a corrupt standalone validator', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(corruptRoot, 'bin/runedraw.mjs'), 'doctor'], {
    cwd: corruptRoot,
    encoding: 'utf8',
  });
  ...
  assert.match(result.stderr, /RuneDraw is not ready: 1 runtime check failed\./);
});
...
test('cli: examples renders from an installed skill', () => {
  ...
  const result = spawnSync(process.execPath, [path.join(installedRoot, 'bin/runedraw.mjs'), 'examples'], {
    cwd: installedRoot,
    encoding: 'utf8',
  });
...
test('cli: guide lists all scenario recipes by diagram type', () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.match(result.stdout, /RuneDraw scenario recipes \(11\)/);
...
test('cli: guide works from an installed skill without node_modules', () => {
  ...
  copyInstalledSkill(installedRoot);
  const installedCli = path.join(installedRoot, 'bin/runedraw.mjs');
...
test('cli: demo creates a ready-to-open diagram in a chosen directory', () => {
  ...
  const output = path.join(outputDirectory, 'runedraw-demo.html');
  ...
  assert.match(result.stdout, /runedraw render architecture/);
});
...
test('cli: demo defaults to the current directory', () => {
  ...
  assert.equal(result.status, 0, result.stderr);
  assert.equal(fs.existsSync(path.join(workingDirectory, 'runedraw-demo.html')), true);
});
...
test('cli: preview runs from an installed skill without node_modules and exits cleanly', { timeout: 30000 }, async () => {
  ...
  assert.deepEqual(fs.readdirSync(path.dirname(output)).filter((name) => name.startsWith('.runedraw-preview-')), []);
});
...
test('cli: deliver preserves the previous artifact when the final check fails', () => {
  ...
  assert.deepEqual(
    fs.readdirSync(path.dirname(out)).filter((name) => name.includes('.runedraw-delivery-')),
    [],
  );
});
...
test('cli: inspect emits architecture layout json', () => {
  const input = path.resolve(skillRoot, '../examples/runedraw-repo-grid.architecture.json');
