...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-semantic-flow-'));
...
test('Semantic Flow has preset identities and motion-safe density boundaries', () => {
  ...
  assert.match(html, /@keyframes runedraw-semantic-lens-flow/);
  assert.match(html, /animation: runedraw-semantic-lens-flow 1\.35s linear 1 both/);
