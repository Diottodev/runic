...
function localMcoRoot() {
  const candidates = [
    process.env.RUNEDRAW_MCO_REPO_ROOT,
...
test('MCO proof is source-backed, reproducible, and linked from every README', () => {
  ...
  assert.match(checkedInHtml, /RuneDraw\.sourceEvidence\.installBeacons\(\)/);
  execFileSync(process.execPath, [cli, 'check', artifactPath], { encoding: 'utf8' });

  const mcoRoot = localMcoRoot();
  if (mcoRoot) {
    const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-mco-proof-'));
    ...
    const output = path.join(os.tmpdir(), `runedraw-mco-proof-no-root-${process.pid}.html`);
