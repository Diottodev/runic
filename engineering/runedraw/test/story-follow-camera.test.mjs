...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-story-follow-camera-'));
...
test('Story Follow frames the exact previous, current, and next authored stops through the shared camera', () => {
  ...
  assert.match(template, /RuneDraw\.view\.reveal\(ids, \{/);
...
test('adaptive dwell, Still, reduced motion, hidden pages, and print keep camera motion bounded', () => {
  ...
  assert.match(template, /RuneDraw\.motionGovernor && RuneDraw\.motionGovernor\.capable\) return !RuneDraw\.motionGovernor\.isPaused\(\)/);
...
test('pause, settle, overview, and manual camera takeover cancel Story Follow state', () => {
  ...
  assert.match(template, /function interruptCamera\(reason\)[\s\S]*?RuneDraw\.guidedViews\.pause\(\)/);
