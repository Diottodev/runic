...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-preset-tryon-'));
...
function presetRuntime(html) {
  return html.match(/RuneDraw\.preset = \(function \(\) \{[\s\S]*?\n    \}\)\(\);/)?.[0] || '';
}
...
test('all five renderers expose one reader-controlled visual style picker', () => {
  for (const mode of Object.keys(CASES)) {
    ...
    assert.match(html, /RuneDraw\.preset = \(function \(\)/, mode);
