...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-webm-artifact-'));
const externalReachSource = process.env.RUNEDRAW_REACH_CARD_SOURCE
  ? path.resolve(process.env.RUNEDRAW_REACH_CARD_SOURCE)
  : '';
const externalReachOutput = process.env.RUNEDRAW_REACH_CARD_OUTPUT
  ? path.resolve(process.env.RUNEDRAW_REACH_CARD_OUTPUT)
  : '';
assert.equal(Boolean(externalReachSource), Boolean(externalReachOutput), 'RUNEDRAW_REACH_CARD_SOURCE and RUNEDRAW_REACH_CARD_OUTPUT must be set together');
...
function executable(candidates) {
  for (const candidate of candidates) {
    ...
    try {
      return execFileSync('sh', ['-c', `command -v "$1"`, 'runedraw-which', candidate], { encoding: 'utf8' }).trim();
    } catch (_) {
...
const ffmpeg = executable([process.env.RUNEDRAW_FFMPEG, 'ffmpeg']);

assert.ok(chrome, 'Chrome/Chromium is required for the WebM artifact smoke test (or set RUNEDRAW_CHROME)');
assert.ok(ffmpeg, 'ffmpeg is required for the WebM artifact smoke test (or set RUNEDRAW_FFMPEG)');
...
try {
  ...
  async function verifySemanticPassportDismissal(file) {
    await navigateReady(file, '!!(window.RuneDraw && RuneDraw.focus && document.querySelector("#btn-focus-clear"))', 'Semantic Passport dismissal');
    const result = await evaluate(cdp, sessionId, `(() => {
      ...
      function state() {
        return { hidden: chip.hidden, active: RuneDraw.focus.active() };
      }

      RuneDraw.focus.set('clients', { toggle: false, updateUrl: false });
  ...
  async function verifyArchitectureDeltaNavigator(file) {
    ...
    const exportProof = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      ...
      var explorers = frames.map(function (frame) {
        ...
        return Boolean(child && child.RuneDraw && child.RuneDraw.focus && child.RuneDraw.routeProbe && child.document.querySelector('#btn-node-finder') && child.document.querySelector('#guided-view-play'));
      });
      var svgA = RuneDraw.deltaExport.canonicalSvg();
      document.querySelector('#theme').click();
      document.querySelector('#preset').click();
      document.querySelector('.change-row').click();
      var svgB = RuneDraw.deltaExport.canonicalSvg();
      var parsed = new DOMParser().parseFromString(svgB, 'image/svg+xml');
      var exportStyle = parsed.querySelector('style')?.textContent || '';
      var blob = await RuneDraw.deltaExport.shareCard();
  ...
  async function captureShareCard(file, label) {
    await navigateReady(file, '!!(window.RuneDraw && RuneDraw.exportMenu && RuneDraw.exportMenu.shareCard)', label);
    const sharePayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        var blob = await RuneDraw.exportMenu.shareCard();
        var bytes = new Uint8Array(await blob.arrayBuffer());
        var binary = '';
  ...
  async function captureCopiedShareCard(file, label) {
    await navigateReady(file, '!!(window.RuneDraw && RuneDraw.exportMenu && RuneDraw.exportMenu.copyShareCard)', label);
        ...
        Object.defineProperty(navigator, 'clipboard', {
          ...
          value: {
            write: async function (items) {
              window.__runedrawCopiedShareCard = await Promise.resolve(items[0].items['image/png']);
            }
          }
        });
        window.alert = function (message) { window.__runedrawCopyAlert = message; };
        await RuneDraw.exportMenu.copyShareCard();
        var blob = window.__runedrawCopiedShareCard;
        if (!blob) throw new Error(window.__runedrawCopyAlert || 'clipboard received no blob');
        var bytes = new Uint8Array(await blob.arrayBuffer());
        var binary = '';
        for (var offset = 0; offset < bytes.length; offset += 32768) {
          binary += String.fromCharCode.apply(null, bytes.subarray(offset, offset + 32768));
  ...
  async function captureRouteShareCard(file, label, sourceId, targetId, options = {}) {
    await navigateReady(file, '!!(window.RuneDraw && RuneDraw.routeProbe && RuneDraw.exportMenu && RuneDraw.exportMenu.downloadRouteShareCard)', label);
    const routePayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        window.alert = function (message) { window.__runedrawRouteAlert = message; };
        RuneDraw.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
        if (!RuneDraw.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) {
          throw new Error('route did not resolve');
        }
        var snapshot = RuneDraw.routeProbe.exportSnapshot();
        if (!snapshot) throw new Error('resolved route exposed no export snapshot');
        RuneDraw.exportMenu.syncRouteShare();
        ...
        var duplicateGeometryRejected = RuneDraw.routeProbe.exportSnapshot() === null;
        var duplicateExportError = '';
        try { await RuneDraw.exportMenu.shareCard({ variant: 'route' }); }
        ...
        var emptyGeometryRejected = RuneDraw.routeProbe.exportSnapshot() === null;
        ...
        try {
          blob = await RuneDraw.exportMenu.downloadRouteShareCard();
          routeReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
          ...
          if (${JSON.stringify(options.journeyInvariance === true)}) {
            async function captureJourneySource(action) {
              ...
              await RuneDraw.exportMenu.shareCard({ variant: 'route' });
              routeFingerprints.push(fingerprint(await captured[captureIndex]));
            }
            await captureJourneySource(function () { RuneDraw.routeProbe.selectJourneyIndex(0); });
            await captureJourneySource(function () { RuneDraw.routeProbe.selectJourneyIndex(Math.floor(snapshot.nodeIds.length / 2)); });
            await captureJourneySource(function () { RuneDraw.routeProbe.selectJourneyIndex(snapshot.nodeIds.length - 1); });
            RuneDraw.routeProbe.showOverview({ reveal: false });
            var started = RuneDraw.routeProbe.playJourney();
            ...
            RuneDraw.routeProbe.pauseJourney({ preserveElapsed: false });
            await captureJourneySource(function () {});
          ...
          await RuneDraw.exportMenu.run('svg');
          ...
          await RuneDraw.exportMenu.run('png');
          var pngReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            canonical: document.documentElement.getAttribute('data-last-export-canonical')
          ...
          var asyncClearPromise = RuneDraw.exportMenu.shareCard({ variant: 'route' });
          RuneDraw.routeProbe.clear({ updateUrl: false, preserveView: true });
          ...
          await RuneDraw.exportMenu.downloadRouteShareCard();
          var failedReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
          ...
          return {
            ...
            staleSnapshot: RuneDraw.routeProbe.exportSnapshot(),
  ...
  async function verifyDynamicReducedMotionRoute(file, label, sourceId, targetId) {
    ...
    await navigateReady(file, '!!(window.RuneDraw && RuneDraw.routeProbe && RuneDraw.exportMenu && RuneDraw.motionGovernor)', label);
    ...
    const setup = await evaluate(cdp, sessionId, `(function () {
      RuneDraw.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
      if (!RuneDraw.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) return { resolved: false };
      RuneDraw.routeProbe.showOverview({ reveal: false });
      return {
        resolved: true,
        started: RuneDraw.routeProbe.playJourney(),
        playing: RuneDraw.routeProbe.isJourneyPlaying(),
  ...
  async function captureRouteVisualMatrix(file, label, sourceId, targetId) {
    await navigateReady(file, '!!(window.RuneDraw && RuneDraw.preset && RuneDraw.routeProbe && RuneDraw.exportMenu)', label);
    const matrix = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      RuneDraw.routeProbe.begin({ source: ${JSON.stringify(sourceId)}, focusNode: false });
      if (!RuneDraw.routeProbe.choose(${JSON.stringify(targetId)}, { updateUrl: false })) {
        throw new Error('route did not resolve');
      }
      var identity = JSON.stringify(RuneDraw.routeProbe.exportSnapshot());
      var results = [];
      for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
        if (!RuneDraw.preset.apply(preset)) throw new Error('could not apply preset ' + preset);
        for (var theme of ['dark', 'light']) {
          document.documentElement.setAttribute('data-theme', theme);
          var blob = await RuneDraw.exportMenu.shareCard({ variant: 'route' });
          var bytes = new Uint8Array(await blob.arrayBuffer());
          var hash = 2166136261;
          ...
          results.push({
            ...
            identity: JSON.stringify(RuneDraw.routeProbe.exportSnapshot())
          });
        }
      }
  ...
  async function captureReachShareCard(file, label, originId, direction, options = {}) {
    await navigateReady(file, '!!(window.RuneDraw && RuneDraw.focus && RuneDraw.focus.reachabilitySnapshot && RuneDraw.exportMenu && RuneDraw.exportMenu.downloadReachShareCard)', label);
    const reachPayload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
      try {
        window.alert = function (message) { window.__runedrawReachAlert = String(message); };
        if (!RuneDraw.focus.set(${JSON.stringify(originId)}, { toggle: false, updateUrl: false })) {
          throw new Error('focus origin did not resolve');
        }
        if (!RuneDraw.focus.reach(${JSON.stringify(direction)}, { toggle: false, updateUrl: false, reveal: false })) {
          throw new Error('authored reach did not resolve');
        }
        var snapshot = RuneDraw.focus.reachabilitySnapshot();
        if (!snapshot) throw new Error('active authored reach exposed no export snapshot');
        RuneDraw.exportMenu.syncReachShare();
        ...
        svg.appendChild(duplicateCarrier);
        var duplicateGeometryRejected = RuneDraw.focus.reachabilitySnapshot() === null;
        var duplicateExportError = '';
        try { await RuneDraw.exportMenu.shareCard({ variant: 'reach' }); }
        catch (error) { duplicateExportError = String(error && error.message || error); }
        duplicateCarrier.remove();
        snapshot = RuneDraw.focus.reachabilitySnapshot();
        ...
        try {
          var blob = await RuneDraw.exportMenu.shareCard({ variant: 'reach' });
          var reachSvgText = captured[0] ? await captured[0] : '';
          var downloadedBlob = await RuneDraw.exportMenu.downloadReachShareCard();
          var reachReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            width: document.documentElement.getAttribute('data-last-export-width'),
          ...
          if (${JSON.stringify(options.matrix === true)}) {
            ...
            for (var preset of ['classic', 'signal-flow', 'blueprint', 'editorial']) {
              ...
              for (var theme of ['dark', 'light']) {
                document.documentElement.setAttribute('data-theme', theme);
                var matrixBlob = await RuneDraw.exportMenu.shareCard({ variant: 'reach' });
                ...
                matrix.push({
                  ...
                  identity: JSON.stringify(RuneDraw.focus.reachabilitySnapshot())
                });
              }
            }
          ...
          RuneDraw.focus.clearReach({ updateUrl: false });
          RuneDraw.exportMenu.syncReachShare();
          ...
          await RuneDraw.exportMenu.downloadReachShareCard();
          var failedReceipt = {
            format: document.documentElement.getAttribute('data-last-export-format'),
            variant: document.documentElement.getAttribute('data-last-export-variant'),
            errorFormat: document.documentElement.getAttribute('data-last-export-error-format'),
          ...
          var canonicalIndex = captured.length;
          var canonicalBlob = await RuneDraw.exportMenu.shareCard();
          var canonicalSvgText = captured[canonicalIndex] ? await captured[canonicalIndex] : '';
          var canonicalSvg = parser.parseFromString(canonicalSvgText, 'image/svg+xml').documentElement;
          ...
          return {
            ...
            hiddenAfterClear: hiddenAfterClear,
            staleSnapshot: RuneDraw.focus.reachabilitySnapshot(),
            staleError: staleError,
            failedReceipt: failedReceipt,
            canonicalSize: canonicalBlob.size,
  ...
  if (externalReachSource) {
    ...
    await captureReachShareCard(
      externalReachSource,
      'external-reach',
      process.env.RUNEDRAW_REACH_CARD_ORIGIN || 'router',
      process.env.RUNEDRAW_REACH_CARD_DIRECTION || 'downstream',
  ...
  await navigateReady(output, '!!(window.RuneDraw && RuneDraw.motion && RuneDraw.motion.canRecord())', 'motion artifact');

  const payload = await withTimeout(evaluate(cdp, sessionId, String.raw`(async function () {
    try {
      var blob = await RuneDraw.motion.recordWebm({ duration: 1400, fps: 12 });
      var bytes = new Uint8Array(await blob.arrayBuffer());
      var binary = '';
      for (var offset = 0; offset < bytes.length; offset += 32768) {
        binary += String.fromCharCode.apply(null, bytes.subarray(offset, offset + 32768));
