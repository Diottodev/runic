...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-diagram-guide-'));
...
test('all typed renderers inherit one viewer-only Diagram Guide', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /RuneDraw\.guide = \(function \(\)/, mode);
    assert.match(html, /Diagram Guide — a factual command deck over existing interactions/, mode);
    assert.doesNotMatch(canonicalSvg(html), /diagram-guide|RuneDraw\.guide|Explore this system/, mode);
  }
});
...
test('Diagram Guide reports compiled semantic facts and honest story availability', () => {
  ...
  assert.match(html, /return RuneDraw\.guidedViews && Number\(RuneDraw\.guidedViews\.count\) \|\| 0/);
...
test('Diagram Guide delegates its task rows to existing production interactions', () => {
  ...
  assert.match(html, /if \(action === 'find'\) return RuneDraw\.finder\.open\(\)/);
  assert.match(html, /if \(action === 'route'\) return RuneDraw\.routeProbe\.begin\(\{ focusNode: true \}\)/);
  assert.match(html, /if \(action === 'map'\) return RuneDraw\.radar\.open\(\)/);
  assert.match(html, /if \(action === 'story'\) return RuneDraw\.guidedViews\.play\(\)/);
  assert.match(html, /if \(action === 'present'\) return RuneDraw\.presentation\.enter\(\)/);
  assert.match(html, /if \(action === 'export'\) return RuneDraw\.exportMenu\.open\(\)/);
  assert.match(html, /if \(action === 'theme'\) return RuneDraw\.theme\.toggle\(\)/);
  assert.match(html, /if \(action === 'reset'\) return RuneDraw\.view\.reset\(\)/);
  assert.match(html, /RuneDraw\.guidedViews\.pause\(\)/);
  assert.match(html, /RuneDraw\.finder\.close\(\{ restoreFocus: false \}\)/);
  assert.match(html, /RuneDraw\.radar\.close\(\{ restoreFocus: false \}\)/);
...
test('Diagram Guide is keyboard-first, mobile-contained, motion-safe, and embed-clean', () => {
  ...
  assert.match(html, /RuneDraw\.guide\.toggle\(\)/);
