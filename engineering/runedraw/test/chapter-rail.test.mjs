...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-chapter-rail-'));
...
test('chapter rail has positional, touch, mobile, motion, embed, and print boundaries', () => {
  ...
  assert.match(html, /\.toolbar, \.diagram-nav, \.focus-chip, \.guided-views, \.runedraw-toast, \.no-print \{ display: none !important; \}/);
});
