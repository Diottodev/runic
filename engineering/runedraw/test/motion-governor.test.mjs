...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-motion-governor-'));
...
test('all renderers inherit one viewer-only Live/Still Motion Governor', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /RuneDraw\.motionGovernor = \(function \(\)/, mode);
...
test('reader pause is persistent, explicit, and reduced-motion aware', () => {
  ...
  assert.match(html, /var STORAGE_KEY = 'runedraw-motion'/);
  ...
  assert.match(html, /RuneDraw\.guidedViews\.isPlaying\(\)[\s\S]*?RuneDraw\.guidedViews\.pause\(\)/);
...
test('strong semantic intent receives the single motion budget', () => {
  ...
  assert.match(html, /animation: runedraw-route-probe-flow 1\.1s[\s\S]*?1 both/);
  assert.doesNotMatch(html, /runedraw-route-probe-flow[^;]*infinite/);
});
