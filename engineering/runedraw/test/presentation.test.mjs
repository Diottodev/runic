...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-presentation-'));
...
test('all typed renderers ship the same presentation stage contract', () => {
  for (const [mode, example] of Object.entries(CASES)) {
    ...
    assert.match(html, /RuneDraw\.presentation = \(function \(\)/, mode);
...
test('presentation keyboard behavior exits in layers and remains accessible', () => {
  ...
  assert.match(html, /RuneDraw\.presentation\.toggle\(\)/);
  assert.match(html, /e\.key === 'Escape' && RuneDraw\.focus\.active\(\)/);
  assert.match(html, /e\.key === 'Escape' && RuneDraw\.presentation\.active\(\)/);
