...
const assetPath = path.join(repoRoot, 'docs', 'assets', 'runedraw-live-proof.gif');
const receiptPath = path.join(repoRoot, 'docs', 'assets', 'runedraw-live-proof.json');
...
test('README motion proof is compact, looping, and backed by current gallery artifacts', () => {
  ...
  assert.equal(receipt.output, 'docs/assets/runedraw-live-proof.gif');
...
test('all README languages keep the product hero and retain the verified animated proof', () => {
  for (const filename of ['README.md', 'README_EN.md', 'README_ZH.md']) {
    ...
    const heroIndex = readme.indexOf('docs/assets/runedraw-readme-hero.png');
    const titleIndex = readme.indexOf('# RuneDraw');
    const proofIndex = readme.indexOf('docs/assets/runedraw-live-proof.gif');
    const demosIndex = Math.max(readme.indexOf('## See RuneDraw in action'), readme.indexOf('## 看看 RuneDraw 能做什么'));
    ...
    assert.match(readme, /docs\/assets\/runedraw-live-proof\.gif/);
    assert.match(readme, /https:\/\/tt-a1i\.github\.io\/runedraw\/gallery\.html/);
  }
  assert.equal(
...
test('README stays scannable without deleting the visual proof set', () => {
  const commonAssets = [
    'runedraw-readme-hero.png',
    'runedraw-live-proof.gif',
    'runedraw-demo-story.png',
    'runedraw-demo-route.png',
    'runedraw-demo-lens.png',
    'mco-runtime-share-card.png',
    'runedraw-dark.png',
    'runedraw-light.png',
    'runedraw-menu.png',
    'runedraw-workflow.png',
    'runedraw-sequence.png',
    'runedraw-dataflow.png',
    'runedraw-lifecycle.png',
  ];
