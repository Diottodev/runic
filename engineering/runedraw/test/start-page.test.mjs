...
test('start page: checked-in HTML is reproducible from canonical scenario recipes', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-start-page-'));
...
test('start page: offers five bounded bilingual starts without ingesting source content', () => {
  ...
  assert.match(html, /npx -y skills add tt-a1i\/runedraw --skill runedraw --agent codex --global --copy --yes/);
  assert.match(html, /npx -y skills add tt-a1i\/runedraw --skill runedraw --agent codex --copy --yes/);
  ...
  assert.match(scriptMatch[1], /runedraw:start-funnel/);
  assert.match(scriptMatch[1], /runedraw\.start\.events\.v1/);
...
test('generated artifacts: viewer-only Create yours link carries the exact diagram type', () => {
  ...
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-start-artifacts-'));
  try {
    for (const [type, input] of Object.entries(examples)) {
      ...
      const expected = `https://diottodev.github.io/runic/start.html?type=${type}&amp;source=artifact`;
