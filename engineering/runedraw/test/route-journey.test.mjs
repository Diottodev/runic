...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-route-journey-'));
...
test('motion, camera, layered Escape, mobile, print, and embed boundaries stay explicit', () => {
  assert.match(template, /RuneDraw\.motionGovernor\.capable === true[\s\S]*?!RuneDraw\.motionGovernor\.isPaused\(\)/);
  assert.match(template, /RuneDraw\.motionGovernor\.claim\('route'/);
  assert.match(template, /reason: 'route-journey',[\s\S]*?maxScale: 1\.65,[\s\S]*?padding: 64,[\s\S]*?duration: 360/);
  assert.match(template, /RuneDraw\.routeProbe\.pauseJourney\(\{ preserveElapsed: true, reason: reason \|\| 'manual' \}\)/);
  ...
  assert.match(template, /RuneDraw\.routeProbe\.escape\(\{ restoreFocus: true \}\)/);
