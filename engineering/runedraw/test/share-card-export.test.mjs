...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-share-card-'));
...
test('Share Card uses contain-only canonical geometry with fixed safe areas', () => {
  ...
  assert.match(html, /RUNEDRAW ·/);
  assert.doesNotMatch(svgBlock(html), /share-card|Share Card|RUNEDRAW ·/);
});
