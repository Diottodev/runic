...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-ordinary-model-floor-'));
...
test('checked-in prompts permit bundled CLI repair while retaining external validation authority', () => {
  ...
  for (const entry of manifest.cases) {
    ...
    assert.match(
      prompt,
      /Use the bundled RuneDraw CLI to validate and repair the candidate when shell access is available\./,
      entry.prompt,
    );
    assert.match(
      prompt,
