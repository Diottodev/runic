...
const cursorCommand = 'npx -y skills add Diottodev/runic --skill runedraw --agent cursor --global --copy --yes';
...
test('Cursor onboarding stays explicit, bilingual, and backed by the same Skill', () => {
  ...
  for (const surface of [english, chinese, start, landing]) {
    ...
    assert.doesNotMatch(surface, /~\/\.cursor\/skills\/runedraw/);
...
test('the zero-dependency archive works from the canonical Cursor-visible agent path', () => {
  const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-cursor-package-'));
  const agentSkills = path.join(tmp, '.agents', 'skills');
  try {
    fs.mkdirSync(agentSkills, { recursive: true });
    execFileSync('unzip', ['-q', path.join(repoRoot, 'runedraw.zip'), '-d', agentSkills]);
    const installed = path.join(agentSkills, 'runedraw');
    const cli = path.join(installed, 'bin', 'runedraw.mjs');
    const doctor = execFileSync(process.execPath, [cli, 'doctor'], { encoding: 'utf8' });
    assert.match(doctor, /RuneDraw is ready\./);
