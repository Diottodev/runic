...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'runedraw-chapter-delta-preview-'));
...
test('pointer and keyboard inspect while touch and native activation still commit directly', () => {
  ...
  assert.doesNotMatch(previewRuntime, /RuneDraw\.view\.|updateUrl\(|RuneDraw\.focus\.|renderStoryTrail\(/);
});
...
test('latest intent, stronger owners, playback, and lifecycle cleanup remain bounded', () => {
  ...
  assert.match(html, /RuneDraw\.motionGovernor\.claim\('chapter-preview'/);
  assert.match(html, /RuneDraw\.motionGovernor\.release\(token\)/);
