...
Render `diagram_type: "sequence"` JSON files into the standard RuneDraw HTML
template.

```bash
node runedraw/renderers/sequence/render-sequence.mjs input.sequence.json output.html
```

The renderer validates input against `runedraw/schemas/sequence.schema.json`
...
`runedraw/examples/cache-miss-request.sequence.json`.

The schema lives at:

```text
runedraw/schemas/sequence.schema.json
```
