...
Render `diagram_type: "lifecycle"` JSON files into the standard RuneDraw HTML
template.

```bash
node runedraw/renderers/lifecycle/render-lifecycle.mjs input.lifecycle.json output.html
```

The renderer validates input against `runedraw/schemas/lifecycle.schema.json`
...
`runedraw/examples/agent-run.lifecycle.json`.

The schema lives at:

```text
runedraw/schemas/lifecycle.schema.json
```
