...
Render `diagram_type: "workflow"` JSON files into the standard RuneDraw HTML
template.

```bash
node runedraw/renderers/workflow/render-workflow.mjs input.workflow.json output.html
```

The renderer validates input against `runedraw/schemas/workflow.schema.json`
...
```bash
node runedraw/scripts/check-render-output.mjs output.html
```
...
`runedraw/examples/agent-tool-call.workflow.json`.

The schema lives at:

```text
runedraw/schemas/workflow.schema.json
```
