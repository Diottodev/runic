...
const SVG_SLOT_RE = /      <!-- RUNEDRAW:SVG_SLOT_START -->[\s\S]*?      <!-- RUNEDRAW:SVG_SLOT_END -->/;
const CARDS_SLOT_RE = /    <!-- RUNEDRAW:CARDS_SLOT_START -->[\s\S]*?    <!-- RUNEDRAW:CARDS_SLOT_END -->/;
const GUIDED_VIEWS_PLACEHOLDER = '<!-- RUNEDRAW:GUIDED_VIEWS_DATA -->';
const SOURCE_EVIDENCE_PLACEHOLDER = '    <!-- RUNEDRAW:SOURCE_EVIDENCE_DATA -->';
...
export function applyTemplate(template, { title, subtitle, footer, svg, cards, visualPreset = 'classic', guidedViews = [], sourceEvidence = null }) {
  if (!SVG_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: template missing RUNEDRAW:SVG_SLOT sentinel');
  }
  if (!CARDS_SLOT_RE.test(template)) {
    throw new Error('applyTemplate: template missing RUNEDRAW:CARDS_SLOT sentinel');
  }
  ...
  return template
    ...
    .replace(GUIDED_VIEWS_PLACEHOLDER, () => `<script id="runedraw-guided-views-data" type="application/json">${guidedViewsJson}</script>`)
    .replace(SOURCE_EVIDENCE_PLACEHOLDER, () => sourceEvidence
      ? `    <script id="runedraw-source-evidence-data" type="application/json">${sourceEvidenceJson}</script>`
      : '');
}
