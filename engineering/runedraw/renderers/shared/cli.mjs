...
export function loadDiagram({ rendererDir, diagramType, defaultExample, argv = process.argv }) {
  ...
  const sourceEvidence = verifyRepositoryEvidence(diagramType, diagram, process.env.RUNEDRAW_REPO_ROOT);
...
export function svgRootAttrs(meta, kind) {
  ...
  const engineeringProfile = meta.engineering_profile
    ...
    : '';
  const requestedProfile = process.env.RUNEDRAW_QUALITY_PROFILE || meta.quality_profile;
  ...
  return `role="img" aria-labelledby="runedraw-diagram-title runedraw-diagram-description"${animation}${preset}${engineeringProfile} data-quality-profile="${esc(qualityProfile)}"${advisory}`;
}

// Keep the accessible name inside the SVG so it survives standalone SVG
// export and embedding. The fixed IDs are deterministic because an RuneDraw
// artifact intentionally contains one primary diagram SVG.
export function svgAccessibleText(meta, kind) {
  const description = meta.subtitle || `A ${kind} generated by RuneDraw.`;
  return `        <title id="runedraw-diagram-title">${esc(meta.title)}</title>\n        <desc id="runedraw-diagram-description">${esc(description)}</desc>`;
}
...
// canonical SVG is embedded inline outside the full RuneDraw viewer.
