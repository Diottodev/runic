...
Render `diagram_type: "dataflow"` JSON files into the standard RuneDraw HTML
template.

```bash
node runedraw/renderers/dataflow/render-dataflow.mjs input.dataflow.json output.html
```

The renderer validates input against `runedraw/schemas/dataflow.schema.json`
...
`runedraw/examples/product-analytics.dataflow.json`.

The schema lives at:

```text
runedraw/schemas/dataflow.schema.json
```
